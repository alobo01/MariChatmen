...
for size in ${MODELS}; do
  case "${size}" in
    08b) model="Qwen/Qwen3.5-0.8B"; slug="qwen_andaluh_08b"; grad=16 ;;
    2b) model="Qwen/Qwen3.5-2B"; slug="qwen_andaluh_2b"; grad=24 ;;
    4b) model="Qwen/Qwen3.5-4B"; slug="qwen_andaluh_4b"; grad=24 ;;
  ...
  MCM_RUN_SLUG="${slug}" \
  MCM_GRAD_ACCUM="${grad}" \
  MCM_CPT_MAX_STEPS="${MCM_CPT_MAX_STEPS:-1000}" \
  MCM_SFT_MAX_STEPS="${MCM_SFT_MAX_STEPS:-1200}" \
  MCM_ORPO_MAX_STEPS="${MCM_ORPO_MAX_STEPS:-400}" \
